...
# =============================================================================
# test-vm.sh — Spin up a QEMU/KVM Fedora VM to test the llm-companion stack
#
...
#   reset     Stop + delete overlay disk (keeps downloaded Fedora image)
...
#   --image PATH      Fedora Cloud Base qcow2      (auto-downloaded if absent)
#   --vm-dir DIR      Directory for VM artifacts   (default: ./vm-test)
#
# What 'start' does:
#   1.  Downloads Fedora 43 Cloud Base image if not already cached
...
#
# After the VM is ready (≈ 10-20 min — dnf upgrade + podman pull):
...
if [[ "${SUBCMD}" == "logs" ]]; then
    ...
    if vm_is_running && ssh_q true 2>/dev/null; then
        ...
        if ssh_q 'test -f ~/.llm-companion-setup-done' 2>/dev/null; then
            ...
            exec ssh ${SSH_OPTS} fedora@127.0.0.1 'journalctl --user -f --no-pager'
        else
            # shellcheck disable=SC2086
            exec ssh ${SSH_OPTS} fedora@127.0.0.1 'sudo journalctl -f --no-pager'
        fi
    fi
...
if [[ "${SUBCMD}" == "console" ]]; then
    ...
    [[ -f "${VM_SSH_KEY}" ]] && SSH_OPTS="${SSH_OPTS} -i ${VM_SSH_KEY}"
    # shellcheck disable=SC2086
    exec ssh ${SSH_OPTS} fedora@127.0.0.1
fi
...
# Filename varies across Fedora releases: discover it from the directory index.
...
if [[ -z "${FEDORA_IMAGE}" ]]; then
    ...
    FEDORA_IMAGE="${VM_DIR}/${IMAGE_NAME}"
fi

if [[ ! -f "${FEDORA_IMAGE}" ]]; then
    IMAGE_URL="${IMAGES_URL}/$(basename "${FEDORA_IMAGE}")"
    step "Downloading Fedora ${FEDORA_VERSION} Cloud Base image (~550 MB)..."
    if wget --show-progress -O "${FEDORA_IMAGE}.tmp" "${IMAGE_URL}"; then
        mv "${FEDORA_IMAGE}.tmp" "${FEDORA_IMAGE}"
    else
        rm -f "${FEDORA_IMAGE}.tmp"
        die "Download failed. Use --image <path> to specify a local image."
    fi
    info "Image saved: ${FEDORA_IMAGE}"
fi
...
if [[ ! -f "${OVERLAY}" ]]; then
    ...
    qemu-img create -f qcow2 -b "$(realpath "${FEDORA_IMAGE}")" -F qcow2 "${OVERLAY}" "${DISK_GB}G"
fi
...
# Runs as root (cloud-init runcmd); user-facing commands run via sudo -u fedora.
...
info "SSH is up — cloud-init setup is running (dnf upgrade + podman pull, 10–20 min)."
...
echo ""
echo -e "${GREEN}══════════════════════════════════════════════════════════════${NC}"
echo -e "${GREEN}  llm-companion test VM is ready!${NC}"
echo -e "${GREEN}══════════════════════════════════════════════════════════════${NC}"
echo ""
...
echo "  Stop:         ./test-vm.sh stop"
echo "  Fresh reset:  ./test-vm.sh reset  (keeps downloaded Fedora image)"
echo ""
